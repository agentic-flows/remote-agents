...
/**
 * Parameters for deduped WebSocket connection
 */
 ...
 * Pattern from Cloudflare's ai-tts-stt example.
 */
export async function dedupedConnect<T extends WebSocket>(
  params: DedupedConnectionParams<T>
): Promise<T> {
  const { getCurrent, setCurrent, getCurrentPromise, setCurrentPromise, connectFn } = params;
