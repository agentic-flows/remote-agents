 ...
 */
import type { LogEntry, LoggerConfig, LogLevel, ObjectContext } from "./types";
...
export class StructuredLogger {
  ...
  private log(
    ...
    if (Object.keys(this.additionalFields).length > 0) {
      const scrubbedAdditionalFields = scrubCredentials(this.additionalFields) as Record<
        string,
        unknown
      >;
...
export class LoggerFactory {
  ...
  static create(component: string): StructuredLogger {
    return new StructuredLogger(component, undefined, LoggerFactory.globalConfig);
  }

  static createForObject(obj: unknown, component?: string): StructuredLogger {
    return createObjectLogger(obj, component, LoggerFactory.globalConfig);
  }
}
